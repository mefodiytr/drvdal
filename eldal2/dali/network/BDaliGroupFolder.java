...
public final class BDaliGroupFolder extends BDeviceFolder implements BallastGroupFolder {
   ...
   public static final Property groupBallastCount;
   ...
   public static final Action assignDaliGroupAddress;
   public static final Action convertToVirtualGroup;
   ...
   public final void assignDaliGroupAddress(BInteger var1) {
      this.invoke(assignDaliGroupAddress, var1, (Context)null);
   }

   public final void convertToVirtualGroup() {
      this.invoke(convertToVirtualGroup, (BValue)null, (Context)null);
   }
   ...
   public final void removed(Property var1, BValue var2, Context var3) {
      BValue var4 = this.get(var1);
      if (var4 instanceof BBallast) {
         BBallast var5 = (BBallast)var4;
         this.getGroupBallastCount().setValue(this.getGroupBallastCount().getValue() - 1.0);
         if (this.isGroupOk() && !this.isVirtualGroup()) {
            this.getDaliNetwork().deviceCommand(var5.getDaliAddress(), 112 + this.getDaliGroupAddress());
         }
      }
      ...
      if (this.isGroupOk()) {
         ...
         if (this.identifyTask == null) {
            if (this.isVirtualGroup()) {
               ...
               this.identifyTask = new IdentifyTask(this.getDaliNetwork(), true, this.getDaliGroupAddress());
            }
   ...
   public final void doAssignDaliGroupAddress(BInteger var1) {
      if (this.isGroupOk()) {
         int var2 = var1.getInt();
         if (var2 >= 1 && var2 <= 16) {
            this.log.message("Changing DALI group address from " + (int)this.getGroupAddress().getValue() + " to " + var2);
            this.getGroupAddress().setValue((double)var2);
            this.getGroupAddress().setStatus(BStatus.ok);
            this.reconfigureGroup(false);
         } else {
            this.log.error("Invalid DALI group address: " + var2);
         }
      }

   }

   public final void doConvertToVirtualGroup() {
      if (this.isGroupOk()) {
         this.getGroupAddress().setValue(0.0);
         this.getGroupAddress().setStatus(BStatus.nullStatus);
         this.reconfigureGroup(false);
      }
   ...
   public final void reconfigureGroup(boolean var1) {
      if (this.isGroupOk()) {
         if (this.devCfgJob != null && this.devCfgJob.isAlive()) {
            this.log.warning("Could not reprogram group " + this.getGroupAddress().getValue() + " a configuration job is already in progress");
            return;
         }
   ...
   private final void programLevel(int var1, BStatusNumeric var2) {
      if (this.isGroupOk() && !var2.getStatus().isNull()) {
         int var3 = DaliArcPowerUtils.percentToDirectLevel(var2.getValue());
         DaliWrite var4;
         if (!this.isVirtualGroup()) {
            var4 = new DaliWrite(this.getDaliNetwork(), true, this.getDaliGroupAddress(), var1, var3);
            var4.setSendRepeat(2);
            var4.setStoreRepeat(2);
            var4.execute();
         } else {
            BComponent[] var5 = this.getChildComponents();

            for(int var6 = 0; var6 < var5.length; ++var6) {
               if (var5[var6] instanceof BBallast) {
                  BBallast var7 = (BBallast)var5[var6];
                  var4 = new DaliWrite(var7.getDaliNetwork(), false, var7.getDaliAddress(), var1, var3);
                  var4.setSendRepeat(2);
                  var4.setStoreRepeat(2);
                  var4.execute();
               }
   ...
   private final void programEnum(int var1, BStatusEnum var2) {
      if (this.isGroupOk() && !var2.getStatus().isNull()) {
         DaliWrite var3;
         if (!this.isVirtualGroup()) {
            var3 = new DaliWrite(this.getDaliNetwork(), true, this.getDaliGroupAddress(), var1, var2.getValue().getOrdinal());
            ...
            for(int var5 = 0; var5 < var4.length; ++var5) {
               if (var4[var5] instanceof BBallast) {
                  ...
                  var3 = new DaliWrite(var6.getDaliNetwork(), false, var6.getDaliAddress(), var1, var2.getValue().getOrdinal());
                  var3.setSendRepeat(2);
                  var3.setStoreRepeat(2);
                  var3.execute();
               }
   ...
   public final void groupCommand(int var1) {
      if (this.isGroupOk()) {
         if (!this.isVirtualGroup()) {
            this.getDaliNetwork().groupCommand(this.getDaliGroupAddress(), var1);
         } else {
            BComponent[] var2 = this.getChildComponents();
            ...
            for(int var3 = 0; var3 < var2.length; ++var3) {
               if (var2[var3] instanceof BBallast) {
                  ...
                  var4.getDaliNetwork().setOpRetransmitCount(var5, var1);
                  var5.execute();
               }
            }
         }
   ...
   public final void groupDirectLevel(double var1) {
      if (this.isGroupOk()) {
         DaliLevel var3;
         if (!this.isVirtualGroup()) {
            var3 = new DaliLevel(this.getDaliNetwork(), true, this.getDaliGroupAddress(), var1);
            ...
            for(int var5 = 0; var5 < var4.length; ++var5) {
               if (var4[var5] instanceof BBallast) {
                  BBallast var6 = (BBallast)var4[var5];
                  var3 = new DaliLevel(var6, var1);
                  var3.setTransmitRepeatCount(var6.getDaliNetwork().getTransmitCount() - 1);
                  var3.execute();
               }
            }
         }
   ...
   public final int getDaliGroupAddress() {
      return (int)this.getGroupAddress().getValue() - 1;
   }
   ...
   public final boolean isVirtualGroup() {
      return this.getGroupAddress().getStatus().isNull() || this.getGroupAddress().getValue() < 1.0 || this.getGroupAddress().getValue() > 16.0;
   }
   ...
   static {
      groupAddress = newProperty(257, new BStatusNumeric(0.0, BStatus.nullStatus), (BFacets)null);
      location = newProperty(256, new BStatusString("", BStatus.nullStatus), (BFacets)null);
      groupBallastCount = newProperty(257, new BStatusNumeric(), (BFacets)null);
      ...
      assignDaliGroupAddress = newAction(0, BInteger.make(1), (BFacets)null);
      convertToVirtualGroup = newAction(0, (BFacets)null);
      reprogram = newAction(128, (BFacets)null);
      TYPE = Sys.loadType(BDaliGroupFolder.class);
   }
}
